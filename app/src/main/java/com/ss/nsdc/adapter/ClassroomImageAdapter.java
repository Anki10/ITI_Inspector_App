...
public class ClassroomImageAdapter extends BaseExpandableListAdapter {


    private Context context;
    private List<String> listDataHeader;
    private Map<String, List<ClassroomImage>> listDataChild;

    public ClassroomImageAdapter(Context context, List<String> listDataHeader,
                                 Map<String, List<ClassroomImage>> listChildData) {
...
//        }
    ...
    public View getChildView(final int groupPosition, final int childPosition, boolean isLastChild, View convertView, ViewGroup parent) {
        final ClassroomImage technicalImage = (ClassroomImage) getChild(groupPosition, childPosition);
...
//        } else {
        LayoutInflater infalInflater = (LayoutInflater) this.context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        convertView = infalInflater.inflate(R.layout.list_instructor, null);
//        }

        TextView textCRPictureType = (TextView) convertView.findViewById(R.id.textInsName);
        final ImageView imgCRImage = (ImageView) convertView.findViewById(R.id.imgInsImage);
        final ImageView imgCRView = (ImageView) convertView.findViewById(R.id.imgInsView);
        final ImageView imageCRRemark = (ImageView) convertView.findViewById(R.id.imageInsRemark);
        final ImageView imageCRNotSubmitted = (ImageView) convertView.findViewById(R.id.imageInsNotSubmitted);
        final ImageView imageCRSubmitted = (ImageView) convertView.findViewById(R.id.imageInsSubmitted);
        ...
        if (technicalImage.getPhoto() != null
            ...
            Picasso.with((Context) context).load(Utility.getTempFile(context, "classroom_image_" + groupPosition + "_" + childPosition))
                    .skipMemoryCache()
                    .resize(100, 100)
...
*/
        ...
        imgCRImage.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (imgCRImage.getTag() != null && ((Integer) imgCRImage.getTag() == R.drawable.upload_pic_36_36)) {
                    ((ClassroomImageActivity) context).selectImage(imgCRImage, groupPosition, childPosition);
                } else {
                    Intent intent = new Intent(context, FullScreenImageViewActivity.class);
                    //if (classroom.isImageChanged()) {
                    intent.putExtra("imgUrl",  "classroom_image_" + groupPosition + "_" + childPosition);
                        /*} else {
        ...
        imageCRRemark.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ((ClassroomImageActivity) context).createRemarkDialog(groupPosition, childPosition);
            }
        });
        ...
        imageCRNotSubmitted.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ((ClassroomImageActivity) context).saveClassrooms(groupPosition, childPosition);
            }
        });
        ...
        imgTechNC.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                ((ClassroomImageActivity)context).changeNC(groupPosition,childPosition,ncValue);
            }
        });
