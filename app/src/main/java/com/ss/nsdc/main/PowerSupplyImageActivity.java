...
import com.ss.nsdc.adapter.PowerImageAdapter;
...
public class PowerSupplyImageActivity extends BaseActivity {

    static int tcIndex;
    ListView lstPowerDetail;
    List<PowerImage> powerDetails;
    List<PowerImage> lstPowerrToBeSync;

    String yearWiseCollegeId = null;
    PowerImageAdapter adapter;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        currentActivity = PowerSupplyImageActivity.this;
        yearWiseCollegeId = getIntent().getStringExtra("yearWiseCollegeId");

        GetPowerImageActivity generalDetailImageActivity = new GetPowerImageActivity();
        generalDetailImageActivity.execute();

        new GetDateandTime().execute();

        lstPowerDetail = (ListView) findViewById(R.id.lstPowerSupplyDetail);
        ...
        actionBar.setTitle("Power Supply Details");
    }
    ...
    public void initDialogViews() {
        ...
        buttonRemark.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (isRemark()) {
                    powerDetails.get(index).setIsRemarkChanged(true);
                    powerDetails.get(index).setRemarks(editRemark.getText().toString());
                    powerDetails.get(index).setProcTracker(2);

                    ITIDBController imagesDAO = new ITIDBController(currentActivity);
                    imagesDAO.updatePowerDetails(powerDetails.get(index));
                    imagesDAO.close();
                    cancelCustomDialog();
                }
            }
    ...
    public void createRemarkDialog(int jobIndex) {
        ...
        if (powerDetails.get(index).getRemarks() != null
                && !powerDetails.get(index).getRemarks().equals("null")) {
            editRemark.setText(powerDetails.get(index).getRemarks());
        } else {
            editRemark.setText("");
        }
        textRemarkType.setText(powerDetails.get(index).getPictureType());
    }
    ...
    public void saveGeneralDetails(int jobIndex) {
        if(Validator.isNetworkAvailable(currentActivity)){
            tcIndex = jobIndex;
              ...
              lstPowerrToBeSync = controller.getUnsavedPowerImageInfobyYearwiseCollegeId(yearWiseCollegeId,powerDetails.get(jobIndex).getDocId());
              controller.close();

            if (lstPowerrToBeSync != null && lstPowerrToBeSync.size() > 0) {
                SavePowerDetailsImages saveGeneralImages = new SavePowerDetailsImages();
    ...
    private void cameraIntent() {
        ...
        intent.putExtra(MediaStore.EXTRA_OUTPUT, Uri.fromFile(Utility.getTempFile(this, "power_image_" + index)));
    ...
    private void onCaptureImageResult(Intent data) {
        ...
        final File file = Utility.getTempFile(this, "power_image_" + index);
        captureBmp = Compressor.getDefault(this).compressToBitmap(file);

        powerDetails.get(index).setIsImageChanged(true);
...
//
//        if(powerDetails.get(index).getPictureType().equalsIgnoreCase("Photograph of Latest Electricity Bill "))
//        {
//            captureBmp.compress(Bitmap.CompressFormat.JPEG, 100, bytes);
//        }else if(powerDetails.get(index).getPictureType().equalsIgnoreCase("Photograph of Affadavit of Power Supply"))
//        {
//            captureBmp.compress(Bitmap.CompressFormat.JPEG, 100, bytes);
//        } else if(powerDetails.get(index).getPictureType().equalsIgnoreCase("Photograph of Resolution of Power Supply"))
//        {
//            captureBmp.compress(Bitmap.CompressFormat.JPEG, 100, bytes);
//        } else {
            captureBmp.compress(Bitmap.CompressFormat.JPEG, 100, bytes);
//        }
        powerDetails.get(index).setPhoto(Base64.encodeToString(bytes.toByteArray(), Base64.DEFAULT));
        powerDetails.get(index).setProcTracker(2);
    ...
    public void emptyImageProp() {
        ...
        for (PowerImage facility : powerDetails) {
            if (facility == null) {
                continue;
            }
        ...
        new AlertDialog.Builder(currentActivity)
                ...
                .setPositiveButton(R.string.submit_later, new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int whichButton) {
                        PowerSupplyImageActivity.this.finish();
                    }
                })
    ...
    public class GetPowerImageActivity extends AsyncTask<String, String, Boolean> {
        ...
        protected Boolean doInBackground(String... params) {
            ITIDBController controller = new ITIDBController(currentActivity);
            powerDetails = controller.getPowerImageInfobyYearwiseCollegeId(yearWiseCollegeId);
            controller.close();
            return true;
        }
        ...
        protected void onPostExecute(Boolean response) {
            ...
            if (response != null && response) {
                if (powerDetails != null) {
                    for (PowerImage generalImageModel : powerDetails) {
                        if (powerDetails == null) {
                            continue;
                        }
                    ...
                    adapter = new PowerImageAdapter(currentActivity, powerDetails);
                    lstPowerDetail.setAdapter(adapter);
    ...
    private class SavePowerDetailsImages extends AsyncTask<String, String, JSONObject> {
        ...
        protected JSONObject doInBackground(String... params) {
            ...
            try {
                ...
                for (int i = 0; i < lstPowerrToBeSync.size(); i++) {
                    JSONObject jsonObject = new JSONObject();
                    jsonObject.put("docId", lstPowerrToBeSync.get(i).getDocId());
                    jsonObject.put("name", lstPowerrToBeSync.get(i).getName());
                    jsonObject.put("photo", lstPowerrToBeSync.get(i).getPhoto());
                    jsonObject.put("remarks", lstPowerrToBeSync.get(i).getRemarks());
                    jsonObject.put("Latitude", lstPowerrToBeSync.get(i).getLatitude());
                    jsonObject.put("Longitude", lstPowerrToBeSync.get(i).getLongitude());
                    jsonObject.put("nc", lstPowerrToBeSync.get(i).getNc());
                    jsonObject.put("refId", lstPowerrToBeSync.get(i).getRefId());

                    jsonArray.put(jsonObject);
                }

                JSONObject GeneralDetailsJsonObj = new JSONObject();
                GeneralDetailsJsonObj.put("PowerSupplyDetails", jsonArray);
                ...
                URL url = new URL(AppConstants.URL_POWERSUPPLY_IMAGE_UPLOAD);
        ...
        protected void onPostExecute(JSONObject responseCode) {
            ...
            if (responseCode != null && responseCode.optString("status").equals("0")) {
                ...
                boolean bolResult = uploadImageDAO.updatePowerProcTracker(lstPowerrToBeSync, yearWiseCollegeId);
                ...
                if (bolResult) {
                    ...
                    Toast.makeText(currentActivity, getString(R.string.alert_power_saved_successfully), Toast.LENGTH_LONG).show();
    ...
    public void changeNC(int position, int ncValue)
    {
        ...
        powerDetails.get(position).setNc(ncValue);
        powerDetails.get(position).setProcTracker(2);

        ITIDBController imagesDAO = new ITIDBController(currentActivity);
        imagesDAO.updatePowerDetails(powerDetails.get(position));
        imagesDAO.close();
        cancelCustomDialog();
        ...
        protected void onPostExecute(JSONObject result) {
            ...
            try {
                ...
                if(status == 0){
                    ...
                    Toast.makeText(PowerSupplyImageActivity.this,"Unable to get the Date and Time",Toast.LENGTH_SHORT).show();
                }
