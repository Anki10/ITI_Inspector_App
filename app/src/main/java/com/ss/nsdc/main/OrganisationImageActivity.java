...
import com.ss.nsdc.adapter.OrganisationImageAdapter;
...
public class OrganisationImageActivity extends BaseActivity {

    static int tcIndex;
    ListView lstOrgDetail;
    List<OrganisationImage> OrgDetails;
    List<OrganisationImage> lstOrgToBeSync;

    String yearWiseCollegeId = null;
    OrganisationImageAdapter adapter;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        actionBar.setTitle("Organisation Details");

        new GetDateandTime().execute();

        currentActivity = OrganisationImageActivity.this;
        yearWiseCollegeId = getIntent().getStringExtra("yearWiseCollegeId");

        GetOrganisationImageActivity organisationImageActivity = new GetOrganisationImageActivity();
        organisationImageActivity.execute();

        lstOrgDetail = (ListView) findViewById(R.id.lstOrgDetail);
        ...
        buttonRemark.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (isRemark()) {
                    ...
                    OrgDetails.get(index).setRemarks(editRemark.getText().toString());
                    OrgDetails.get(index).setProcTracker(2);

                    ITIDBController imagesDAO = new ITIDBController(currentActivity);
                    imagesDAO.updateOrganisationDetails(OrgDetails.get(index));
                    imagesDAO.close();
                    cancelCustomDialog();
                }
            }
    ...
    public void createRemarkDialog(int jobIndex) {
        ...
        if (OrgDetails.get(index).getRemarks() != null
                && !OrgDetails.get(index).getRemarks().equals("null")) {
            editRemark.setText(OrgDetails.get(index).getRemarks());
        } else {
            editRemark.setText("");
        }
        textRemarkType.setText(OrgDetails.get(index).getPictureType());
    }
    ...
    public void changeNC(int position, int ncValue)
    {
        ...
        //OrgDetails.get(index).setIsRemarkChanged(true);
        OrgDetails.get(position).setNc(ncValue);
        OrgDetails.get(position).setProcTracker(2);

        ITIDBController imagesDAO = new ITIDBController(currentActivity);
        imagesDAO.updateOrganisationDetails(OrgDetails.get(position));
        imagesDAO.close();
        cancelCustomDialog();
    ...
    public void saveGeneralDetails(int jobIndex) {
        if(Validator.isNetworkAvailable(currentActivity)){
            ...
            lstOrgToBeSync = controller.getUnsavedOrganisationImageInfobyYearwiseCollegeId(yearWiseCollegeId,OrgDetails.get(jobIndex).getDocId());
            controller.close();

            if (lstOrgToBeSync != null && lstOrgToBeSync.size() > 0) {
                SaveOrgDetailsImages saveGeneralImages = new SaveOrgDetailsImages();
    ...
    private void cameraIntent() {
        ...
        intent.putExtra(MediaStore.EXTRA_OUTPUT, Uri.fromFile(Utility.getTempFile(this, "org_image_" + index)));
    ...
    private void onCaptureImageResult(Intent data) {
        ...
        final File file = Utility.getTempFile(this, "org_image_" + index);
        captureBmp = Compressor.getDefault(this).compressToBitmap(file);

        OrgDetails.get(index).setIsImageChanged(true);
        ...
        OrgDetails.get(index).setPhoto(Base64.encodeToString(bytes.toByteArray(), Base64.DEFAULT));
        OrgDetails.get(index).setProcTracker(2);
    ...
    public void emptyImageProp() {
        ...
        for (OrganisationImage facility : OrgDetails) {
            if (facility == null) {
                continue;
            }
        ...
        new AlertDialog.Builder(currentActivity)
                ...
                .setPositiveButton(R.string.submit_later, new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int whichButton) {
                        OrganisationImageActivity.this.finish();
                    }
                })
    ...
    public class GetOrganisationImageActivity extends AsyncTask<String, String, Boolean> {
        ...
        protected Boolean doInBackground(String... params) {
            ITIDBController controller = new ITIDBController(currentActivity);
            Log.e("yearwise",yearWiseCollegeId);
            OrgDetails = controller.getOrganisationImageInfobyYearwiseCollegeId(yearWiseCollegeId);
            controller.close();
            return true;
        }
        ...
        protected void onPostExecute(Boolean response) {
            ...
            if (response != null && response) {
                if (OrgDetails != null) {
                    for (OrganisationImage generalImageModel : OrgDetails) {
                        if (OrgDetails == null) {
                            continue;
                        }
                    ...
                    adapter = new OrganisationImageAdapter(currentActivity, OrgDetails);
                    lstOrgDetail.setAdapter(adapter);
    ...
    private class SaveOrgDetailsImages extends AsyncTask<String, String, JSONObject> {
        ...
        protected JSONObject doInBackground(String... params) {
            ...
            try {
                ...
                for (int i = 0; i < lstOrgToBeSync.size(); i++) {
                    JSONObject jsonObject = new JSONObject();
                    jsonObject.put("docId", lstOrgToBeSync.get(i).getDocId());
                    jsonObject.put("name", lstOrgToBeSync.get(i).getName());
                    jsonObject.put("photo", lstOrgToBeSync.get(i).getPhoto());
                    jsonObject.put("remarks", lstOrgToBeSync.get(i).getRemarks());
                    jsonObject.put("Latitude", lstOrgToBeSync.get(i).getLatitude());
                    jsonObject.put("Longitude", lstOrgToBeSync.get(i).getLongitude());
                    jsonObject.put("nc", lstOrgToBeSync.get(i).getNc());
                    jsonObject.put("refId", lstOrgToBeSync.get(i).getRefId());

                    jsonArray.put(jsonObject);
                }

                JSONObject GeneralDetailsJsonObj = new JSONObject();
                GeneralDetailsJsonObj.put("OrganisationDetails", jsonArray);
                ...
                URL url = new URL(AppConstants.URL_ORGANISATION_IMAGE_UPLOAD);
        ...
        protected void onPostExecute(JSONObject responseCode) {
            ...
            if (responseCode != null && responseCode.optString("status").equals("0")) {
                ...
                boolean bolResult = uploadImageDAO.updateOrganisationProcTracker(lstOrgToBeSync, yearWiseCollegeId);
                ...
                if (bolResult) {
                    ...
                    Toast.makeText(currentActivity, getString(R.string.alert_org_saved_successfully), Toast.LENGTH_LONG).show();

                    GetOrganisationImageActivity generalDetailImageActivity = new GetOrganisationImageActivity();
        ...
        protected void onPostExecute(JSONObject result) {
            ...
            try {
                ...
                if(status == 0){
                    ...
                    Toast.makeText(OrganisationImageActivity.this,"Unable to get the Date and Time",Toast.LENGTH_SHORT).show();
                }
